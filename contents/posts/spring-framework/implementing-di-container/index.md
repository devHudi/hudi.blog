...
stage3에서 만든 `DIContainer` 도 충분히 쓸만하지만, 직접 `DIContainer` 에 빈으로 등록할 타입을 추가해줘야하는 번거로움이 남아있다. MVC 프레임워크 만들기 미션에서 했던 것 처럼 애노테이션을 사용하면 조금 더 사용성이 좋아질 것 같다. 스프링과 같이 애노테이션 기반으로 동작하는 `DIContainer` 를 작성해보자. 빈으로 지정할때 사용하는 애너테이션은 `@Service` 와 `@Repository` 이 두가지이고, 의존성 주입을 사용할 필드에는 `@Inject` 라는 애노테이션을 사용할 것이다. 즉, 아래와 같이 사용하는 것이 목표이다.
...
사실 크게 달라진 점은 없다. 단지, 클래스를 불러올 때 `@Service` , `@Repository` 애노테이션이 붙은 클래스만을 인스턴스화 하고, 필드 주입 대상으로 `@Inject` 애노테이션이 붙은 필드만 선택한다. 이 결과 직접 빈으로 등록할 클래스의 타입을 추가하지 않고, 애노테이션만 붙여주어 손쉽게 사용할 수 있게 사용성이 개선되었다.
